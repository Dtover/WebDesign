function loadcomment(){
	var str = localStorage.getItem("gc5");
	var gc5 = JSON.parse(str);
	for(var i = 0; i < Number(localStorage.getItem("counter5")); i++)	{
		if(gc5[i] != null){
			var ctag = document.getElementsByClassName("Comment")[0];
			var addc = document.createElement("div");
			var node = document.createTextNode(gc5[i].comment);
			addc.append(node);
			var commentdate = document.createElement("span");
			commentdate.append(gc5[i].date);
...
picnum[1] = "../Mainpage/Gimgs/Grand5/01.jpg";
picnum[2] = "../Mainpage/Gimgs/Grand5/02.jpg";
picnum[3] = "../Mainpage/Gimgs/Grand5/03.jpg";
picnum[4] = "../Mainpage/Gimgs/Grand5/04.jpg";
...
function Like(){
	...
	var gameinfo = new Gameinfo("侠盗猎车手5", "Grand Theft Auto 5", "9.6", "PS4,XBOX,PC", "Grand5");
...
function AddComment(){
	...
	if(comment == ""){
		...
		var isexist = localStorage.getItem("gc5");
		if(isexist == null){
			var gc5 = new Array();
		}else{
			var gc5 = JSON.parse(isexist);
		}
		var i = Number(localStorage.getItem("counter5"));
		var time = new Date();
		...
		addc.append(commentdate);
		ctag.append(addc);
		gc5[i] = new obj(comment, date, "Grand Theft Auto 5");
		localStorage["gc5"] = JSON.stringify(gc5);
		localStorage["counter5"] = Number(localStorage.getItem("counter5")) + 1;
